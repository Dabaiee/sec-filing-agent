"""10-K Annual Report Analyzer — hybrid XBRL + LLM approach."""
...
class TenKAnalyzer(BaseAnalyzer):
    ...
    - LLM for qualitative analysis (risk factors, MD&A, forward-looking)
    """
    ...
    async def analyze(
        ...
        if filing.filing_obj is not None:
            ...
            try:
                ...
                pass  # Fall back to LLM extraction
        ...
        # Stage 3: Risk factors (HIGH complexity — Sonnet)
        if on_stage_start:
        ...
        # Stage 4: LLM financial extraction (only if XBRL failed)
